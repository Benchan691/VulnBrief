...
def patch_iter_collection_documents(monkeypatch, views, documents=None, on_query=None):
    ...
    monkeypatch.setattr('reviews.query._iter_collection_documents', iter_collection_documents)
...
def patch_query_review_slice(monkeypatch, views, documents=None, on_query=None):
    ...
    monkeypatch.setattr('reviews.query._query_review_slice', query_review_slice)
...
def test_review_collection_page_renders(client):
    ...
    assert b'/static/js/reviews/index.js' in main_page.data
    assert b'id="page-config"' in main_page.data
...
def test_review_search_includes_selection_score(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', FakeDatabase)
    monkeypatch.setattr('reviews.routes._review_views', lambda database: views)
    patch_cve_search_data(monkeypatch, views, documents)
    monkeypatch.setattr('reviews.routes._attach_related_cve_documents', lambda *args: None)
...
def test_auto_select_rejects_scan_over_limit(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.AUTO_SELECT_SCAN_LIMIT', 2)
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', FakeDatabase)
    monkeypatch.setattr('reviews.routes._review_views', lambda database: views)
    patch_iter_collection_documents(monkeypatch, views, documents)
...
def test_original_document_export_preserves_order(client, monkeypatch):
    authenticate(client)
    views = {
    ...
    source_documents = {
        ('avd', 'avd:1'): {'_id': 'avd:1', 'details': {'avd': {}}, 'source': {'provider': 'avd'}},
        ('hkcert', 'hkcert:1'): {
            '_id': 'hkcert:1',
            'details': {'hkcert': {}},
    ...
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', lambda: object())
    monkeypatch.setattr('reviews.routes._review_views', lambda database: views)
    monkeypatch.setattr(
        'reviews.routes.resolve_vulnerability_document',
        lambda database, source, selection_id: source_documents.get((source, selection_id)),
    )

    response = client.post('/api/reviews/export-json', json={
        'selections': [
            {'collection': 'hkcert_review', 'selection_id': 'hkcert:1'},
            {'collection': 'avd_review', 'selection_id': 'avd:1'},
        ],
    })
    ...
    assert [document['_id'] for document in documents] == ['hkcert:1', 'avd:1']
...
def test_review_search_mode_cve_includes_vendor_product(client, monkeypatch):
    authenticate(client)
    views = {
        'cve_review': {'options': {'viewOn': 'cve', 'pipeline': [{'$project': {'title': 1}}]}},
    }
        ...
        'cve_review': [
            {
                ...
                'title': 'CVE without vendor',
            },
            {
                '_id': 'cve:2',
                'scraped_at': '2026-06-17T12:00:00',
                'title': 'CVE with vendor',
                'vendor': 'Red Hat',
    ...
    assert 'classification' not in data[0]['document']
    assert data[1]['document']['vendor'] == 'Red Hat'
    assert data[1]['document']['product'] == 'Enterprise Linux'
...
def test_review_search_mode_cve_includes_related_records(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', FakeDatabase)
    monkeypatch.setattr('reviews.routes._review_views', lambda database: views)
    monkeypatch.setattr('reviews.routes._query_review_slice', query_slice)
    monkeypatch.setattr('reviews.query._query_review_slice', query_slice)
...
def test_review_search_keyword_matches_affected_vendor_product(client, monkeypatch):
    authenticate(client)
    views = {
        'cve_review': {'options': {'viewOn': 'cve', 'pipeline': [{'$project': {'title': 1}}]}},
    }
...
def test_review_documents_sort_newest_first(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', FakeDatabase)
    monkeypatch.setattr('reviews.routes._review_views', lambda database: {'cve_review': view})
...
def test_global_review_search_allows_empty_filters_and_rejects_invalid_filters(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes._review_views', lambda database: {})
    response = client.get('/api/reviews/search')
    assert response.status_code == 200
...
def test_global_review_search_allows_over_export_limit(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.MAX_EXPORT_SELECTIONS', 2)
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', FakeDatabase)
    monkeypatch.setattr('reviews.routes._review_views', lambda database: views)
    patch_cve_search_data(monkeypatch, views, documents)
...
def test_review_api_handles_database_failure(client, monkeypatch):
    ...
    monkeypatch.setattr('reviews.routes.get_vulnerabilities_database', unavailable_database)
    response = client.get('/api/reviews')
