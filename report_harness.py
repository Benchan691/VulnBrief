...
class OpenAICompatibleProvider:
    retains_conversation_context = False

    def __init__(self, config, prefix, provider_label):
        self.prefix = prefix
        self.provider_label = provider_label
        self.base_url = config[f'{prefix}_BASE_URL'].rstrip('/')
        self.api_key = config[f'{prefix}_API_KEY']
        self.model = config[f'{prefix}_MODEL']
        self.timeout = config[f'{prefix}_TIMEOUT_SECONDS']
        self.max_output = config[f'{prefix}_MAX_OUTPUT_TOKENS']
        self.retries = config[f'{prefix}_JSON_RETRIES']
        self.json_error_message = config.get('REPORT_JSON_ERROR_MESSAGE', DEFAULT_JSON_ERROR_MESSAGE)

    def _validate_config(self):
        required = {
            f'{self.prefix}_BASE_URL': self.base_url,
            f'{self.prefix}_API_KEY': self.api_key,
            f'{self.prefix}_MODEL': self.model,
        }
        missing = [key for key, value in required.items() if not value]
        if missing:
            raise ProviderError(
                f'{self.provider_label} configuration is missing: {", ".join(missing)}.',
            )
    ...
    def complete_json(self, system_prompt, user_prompt):
        ...
        for _ in range(self.retries + 1):
            try:
                answer = self._completion(messages)
                messages.append({'role': 'assistant', 'content': answer})
                return _parse_json_text(answer), {}
            except requests.RequestException as exc:
                error = exc
            except (KeyError, ValueError, ProviderError) as exc:
                error = exc
            messages.append({'role': 'user', 'content': json_error_prompt(self, error)})
